...
import com.javalego.poi.report.object.elements.Header;
...
public class ReportObject {
	...
	/**
	 * Objeto de donde se obtienen los valores de sus propiedades para contruir el informe.
	 */
	...
	/**
	 * Generar un nombre de archivo a grabar en el directorio temporal del usuario y asignarlo al fileName del report.
	 * 
	 * @return
	 */
	public String generateFileNameTmp() {
		for (int i = 0; i < 100; i++) {
			File file = new File(EnvironmentVariables.getUserTmp() + "tmp" + i + ".xlsx");
			// Intentar eliminarlo
			if (file.exists())
				try {
					file.delete();
	...
	private void writeSections() throws Exception {
		...
		if (sections.size() == 0)
			throw new Exception("Not sections");
	...
	private void addSeparator() throws Exception {
		...
		Cell cell = excel.getCell(getRowIndex(), getColumnIndex());
