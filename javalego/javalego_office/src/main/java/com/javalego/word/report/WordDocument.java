...
import com.javalego.word.report.elements.BasicElementWordDocument;
import com.javalego.word.report.elements.Bookmark;
import com.javalego.word.report.elements.BookmarkImage;
import com.javalego.word.report.elements.CheckBox;
import com.javalego.word.report.elements.MergeField;
import com.javalego.xls.report.ReportVector;
...
public class WordDocument extends ArrayList<BasicElementWordDocument> {
	...
	/**
	 * Nombre del fichero de salida utilizado de forma temporal a la hora de generar el documento.
	 */
	...
	/**
	 * Permite realizar la utilidad de MS-Word de "Combinar correspondencia" para generar un documento de word con una lista de registros. // t.,.2
	 */
	private boolean mailMerge;
	
	/**
	 * Nombre de la vista de Fuente de datos de donde obtiene la información para construir el informe.
	 */
	...
	/**
	 * Máscara utilizada para crear el nombre del fichero. Se pueden utilizar expresiones del tipo $F{campo}
	 * con la información contenida en el objeto asociado al documento.
	 */
	...
	/**
	 * Objeto utilizado en la elaboración del document. Cuando se pasa una lista de objetos, este objeto va cambiando a medida 
	 * que generamos los documentos cada uno de sus elementos.
	 */
...
//	}
	...
	public void execute(boolean writeFile) throws Exception {
		...
		// Si es un lista de objetos hay que crear tantos documentos como elementos de la lista.
	...
	/**
	 * Establecer el nombre del fichero de salida y obtener el fichero plantilla.
	 * @throws Exception
	 */
	...
	/**
	 * Obtener el objeto en el que se base el documento de word para establecer los valores de los diferentes bookmarks.
	 * @return
	 */
	...
	private void execute(Object object, boolean quit) throws Exception {
		...
		// Incluir información en el documento mediante los items o acciones definidas.
		if (wordApp == null) {
...
//						Object tmpValue = new ObjectExpression(object, checkBox.getExpression(), checkBox.isNatural(), checkBox.getDisplayFormat()).getValue();
//						if (tmpValue != null) {
//							if (tmpValue instanceof Number)
//								value = ((Number)tmpValue).intValue() == 0 ? false : true;
//							else
//								value = "true".equals(tmpValue.toString().toLowerCase()) ? true : false;
//
				...
				// Bookmark
					...
					try {
						...
						if (bookmark instanceof BookmarkImage) 
							((BookmarkImage)bookmark).addImage(getBookmarkImageValue((BookmarkImage)bookmark), bookmark.getName(), wordApp, document);
						else {
							document.getBookmark(bookmark.getName()).setText(getBookmarkValue(bookmark));
							//System.out.println(bookmark + "  =  " + getBookmarkValue(bookmark));
						}
					}
					catch(Exception e) {
...
//			value = bookmark.getStringValue(new ObjectExpression(objectDocument, bookmark.getExpression(), bookmark.isNatural(), bookmark.getDisplayFormat()).getValue());
	...
	public Object getBookmarkImageValue(BookmarkImage bookmark) throws Exception {
		...
		// Expresión de cálculo basada en propiedades del objeto
//		else if (bookmark.getExpression() != null) 
//			value = new ObjectExpression(objectDocument, bookmark.getExpression(), bookmark.isNatural(), bookmark.getDisplayFormat()).getValue();
	...
	/**
	 * Generar un documento con la funcionalidad de "Combinación de correspondencia" de MS-Word en base a una lista de registros.
	 * @throws Exception
	 */
	@SuppressWarnings("unchecked")
	private void executeMailMerge() throws Exception {

		// Crear un informe para generar una hoja de excel con la información requerida por el documento MS-Word donde se han definido los campos
		...
		// Ponemos useTemplate = false para que funcione correctamente la combinación de correspondencia sobre
		// una hoja de excel nueva y no usando el plantilla que tenemos estándar para aumentar la resolución a 600ppp.
		report.setUseTemplate(false);
  		...
  		for(BasicElementWordDocument item : this) {
  			...
  			if (item instanceof BookmarkImage) {
  				BookmarkImage bookmark = (BookmarkImage)item;
 					((BookmarkImage)bookmark).addImage(getBookmarkImageValue(bookmark), bookmark.getName(), w, document);
				}
  		}
