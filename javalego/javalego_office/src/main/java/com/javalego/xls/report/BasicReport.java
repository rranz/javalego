...
import com.javalego.xls.report.elements.ICustomReport;
import com.javalego.xls.report.elements.ReportField;
import com.javalego.xls.report.elements.ReportFields;
import com.javalego.xls.report.elements.ReportGroup;
import com.javalego.xls.report.elements.ReportGroups;
import com.javalego.xls.report.elements.ReportHeader;
import com.javalego.xls.report.elements.ReportHeaderFields;
import com.javalego.xls.report.elements.ReportHeaderFieldsEditor;
import com.javalego.xls.report.elements.ReportSeparatorField;
import com.javalego.xls.report.elements.ReportTotalField;
import com.javalego.xls.report.elements.ReportTotalFields;
...
public abstract class BasicReport {
	...
	/**
	 * Personalización del informe mediante la implementación de la interface
	 * customReport.
	 */
	private ICustomReport customReport;
	...
	public String generateFileNameTmp() {
		...
		if (dir.list().length > 0) {
			...
			for (String filename : dir.list()) {
				...
				if (filename.indexOf("gana_tmp_") > -1) {
					...
					File file = new File(EnvironmentVariables.getUserTmp()
							+ filename);
		...
		fileName = EnvironmentVariables.getUserTmp() + "gana_tmp_report_"
				+ SystemUtils.getIdDocument() + ".xls";
	...
	public ExcelWorkbook getExcel() {
		if (excel == null) {
			excel = new ExcelWorkbook(fileName, useTemplate);
		}
		return excel;
	}
			...
			for (int i = 0; i < fields.size(); i++) {
				...
				for (int k = 0; k < groups.size(); k++) {
					...
					for (int j = 0; j < group.getTotalFields().size(); j++) {
						...
						if (((ReportTotalField) group.getTotalFields().get(j))
								.getField() == field)
							exist = true;
					}
				}
	...
	protected void configColumns() {
		...
		for (int i = 0; i < groups.size(); i++)
			if (showDetail)
				...
				getExcel().setColumnWidth(i,
						((ReportGroup) groups.get(i)).getMaxWidth());
		...
		for (int i = 0; i < fields.size(); i++) {
			...
			if (field.isVisible()) {
				...
				if (field.getDisplayWidth() > 0) {
					getExcel().setColumnWidthCharacters(
							getInitColumnIndex() + cont,
		...
		if (cont > 1 && header.hasLogos()) {
			if (header.getTitleRow() > -1)
				excel.mergeCells(header.getTitleRow(), getInitColumnIndex(),
						header.getTitleRow(), getInitColumnIndex() + cont - 1);
			if (header.getSubtitleRow() > -1)
				excel.mergeCells(header.getSubtitleRow(), getInitColumnIndex(),
						header.getSubtitleRow(), getInitColumnIndex() + cont
								- 1);
		}
		...
		try {
			...
			if (header.getSubtitleRow() > -1)
				excel.setRowHeight(header.getSubtitleRow(), 400);
	...
	protected void configPage() {
		...
		if (header.hasRightLogo())
			excel.getActiveSheet().setMargin(HSSFSheet.RightMargin, 0);
	...
	protected void writeFooter() {
		...
		if (showNumberPages)
			footer.setRight("Pages: " 
					+ HSSFFooter.page() + " - "
					+ HSSFFooter.numPages()); // .,.r
	}
	...
	protected void writeTotalFields() throws Exception {
		...
		if (isShowTotals()) {
			...
			totalFields
					.generate(groups != null && groups.size() > 0 ? HSSFColor.ORANGE.index
							: (short) -1);
		}
		...
		if (showRecordCount) {
			...
			getExcel().setCellValue(
					cell,
					(textTotalRecords != null ? textTotalRecords + ": "
	 ...
	 * @throws Exception
	 */
	public abstract Object getFieldValue(Object dataObject, String fieldName)
			throws Exception;
	...
	public ICustomReport getCustomReport() {
		return customReport;
	}

	public void setCustomReport(ICustomReport customReport) {
