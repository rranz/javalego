...
public class ReportGroup implements Serializable {
	 ...
	 */
	private ReportVisualStyle styleRecordCount;
	...
	public ReportGroup(BasicReport report) {
		...
		styleHeader.setFontSize(report.getExcel().createFontStyle().getFontHeightInPoints() + 1);
		
		styleRecordCount = new ReportVisualStyle(report.getExcel());
		styleRecordCount.setFontSize(report.getExcel().createFontStyle().getFontHeightInPoints() - 3);
	...
	public void addGroupField(ReportField field, boolean hideField) {
		...
		groupFields.add(new ReportGroupField(report, field, hideField));
	}
...
//	}
	...
	public void generateHeaders(int columnIndex) throws Exception {
		...
		if (report.isShowDetail()) {
			...
			if (styleHeader.getAlignment() < 0) styleHeader.setAlignment(HSSFCellStyle.ALIGN_LEFT);
			...
			if (styleHeader.getBackgroundColor() < 0) styleHeader.setBackgroundColor(HSSFColor.BLUE.index);
			if (styleHeader.getFontName() == null) styleHeader.setFontName(report.getFontNameDefault());
		}
		else{
			styleHeader.setBox(true);
			if (styleHeader.getForeColor() < 0 || !report.isShowDetail()) styleHeader.setForeColor(HSSFColor.WHITE.index);
			if (styleHeader.getBackgroundColor() < 0 || !report.isShowDetail()) styleHeader.setBackgroundColor(HSSFColor.CORNFLOWER_BLUE.index);
		}
	...
	public void addGroupRows(){
		...
		if (rowInit > -1){
			...
			HSSFSheet sheet = report.getExcel().getActiveSheet();
	...
	public void generateTotalFields() throws Exception {
		...
		if (isShowTotals()) {
			...
			if (isShowRecordCount() && !isShowPercent()){
				...
				HSSFCell cell = report.getExcel().getCell(report.getRowIndex(), columnIndex);
				cell.setCellValue(new HSSFRichTextString((report.getTextTotalRecords() != null ? report.getTextTotalRecords() : "Records" + ": " + recordCount)));
			...
			if (isShowPercent()) {
				...
				cell.setCellValue(new HSSFRichTextString((isShowRecordCount() ? "Records" + ": " + recordCount + "     " : "" ) + new Double(recordCount) * 100 / new Double(report.getDataSize()) + " % "));
				styleHeader.setApplyStyle(cell);
			}
