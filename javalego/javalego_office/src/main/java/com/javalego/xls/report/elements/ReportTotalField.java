...
public class ReportTotalField implements IReportItem, IReportConstans
{
	...
	private void internalTotalize(double value) {
		...
		if (FORMULA_MIN.equals(formula)) {
			if (value < total || total == 0) total = value;
		}
		else if (FORMULA_MAX.equals(formula)) {
			if (value > total || total == 0) total = value;
		}
		else if (FORMULA_PERCENT.equals(formula)) {
			total += value;
		}
		else {	
			total += value;
		...
		if (value != 0.0 || FORMULA_PERCENT.equals(formula))
			++count;
	}
	...
	public void generate(short backgroundColor) throws Exception {
		...
		if (FORMULA_PERCENT.equals(formula) || style.isPercentFormat() || field.getStyleCell().isPercentFormat()) {
		...
		if (FORMULA_PERCENT.equals(formula)) {
