...
import com.javalego.ui.binding.bean.MethodProperty.MethodException;
...
public class NestedMethodProperty<T> extends AbstractProperty<T> {
	...
	private static final long serialVersionUID = -8864762333532202782L;
	...
	private void initialize(Class<?> beanClass, String propertyName) throws IllegalArgumentException {
		...
		this.type = (Class<? extends T>) MethodProperty.convertPrimitiveType(type);
	...
	protected void invokeSetMethod(T value) {
		try {
			...
			// TODO Para las propiedades nested ej.: proveedor.nombre donde
			// proveedor == null.
			// object == null y no se grabaría el cambio del parámetro T value.
			// Hay que intentar forzar
			// la creación del objeto o bean nested antes de aplicar los
			// cambios.
			if (object != null && value != null && propertyName != null && propertyName.indexOf(".") > 0) {
				ReflectionUtils.getPropertyValue(object, propertyName);
			}
			...
			// ROBERTO RANZ CAMBIO 2014: No establecer valor del campo si no
			// existe el objeto nested
			if (object != null) {
				setMethod.invoke(object, new Object[] { value instanceof String && "".equals(value) ? null : value });
			}
