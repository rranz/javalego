...
/**
 * Una colección de utilidades para dibujar efectos visuales.
...
class PaintUtils {
  ...
  public static void setFont(Container cont, Font font) {
	...
	/**
	 * Ajustar el tamano del font de un formulario.
	 * @param size
	 * @param frame
	 */
	static public void setFont(final double size, JFrame frame) {
		...
		for(int i = 0;i < objs.length;i++) {
			if(objs[i].toString().toUpperCase().indexOf("FONT") != -1) {
				...
				UIManager.put(objs[i], new FontUIResource(font.deriveFont((float)(size))));
			}
		}
